...
function Header() {
  ...
  return (
    <nav id="top" className={`${scrolled ? 'bg-primary/80' : 'bg-primary border-b border-primary'
      } fixed top-0 w-full items-center py-10 px-20 z-50`}>
      <div className="flex justify-between  md:hidden sm:hidden">
      ...
      {menuOpen && (
        <div className="absolute top-16 left-0 w-full bg-primary flex flex-col items-center gap-7 text-2xl py-4">
          <Link
            to="about"
            smooth={true}
            duration={1000}
            ...
            className={`${activeLink === 'about' ? 'text-neutral-500' : 'text-accent'
              } text-2xl font-bold flex items-center cursor-pointer hover:text-neutral-500`}
            onSetActive={() => {
              setActiveLink('about');
              setMenuOpen(false);
            }}
